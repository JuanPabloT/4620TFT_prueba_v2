...
x = *addr++;
numArgs = x & 0x7F;
while(numArgs--) SPI1_ExchangeByte(*addr++);

if(x & 0x80) _delay((unsigned long)((150)*(32000000/4000.0)));
cmd = *addr++;
}
...
y2 = (y1 + h - 1);
writeCommand(0x2A);
SPI1_ExchangeByte(x1 >> 8);
SPI1_ExchangeByte(x1);
SPI1_ExchangeByte(x2 >> 8);
SPI1_ExchangeByte(x2);
writeCommand(0x2B);
SPI1_ExchangeByte(y1 >> 8);
SPI1_ExchangeByte(y1);
SPI1_ExchangeByte(y2 >> 8);
SPI1_ExchangeByte(y2);
writeCommand(0x2C);
}
...
SPI1_ExchangeByte(color >> 8);
SPI1_ExchangeByte(color & 0xFF);
endWrite();
}
}
...
while (w--) {
SPI1_ExchangeByte(hi);
SPI1_ExchangeByte(lo);
}
endWrite();
}
}
